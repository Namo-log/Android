...
import com.mongmong.namo.data.remote.category.GetCategoryResult
...
import com.mongmong.namo.ui.bottom.home.HomeFragment
import com.mongmong.namo.ui.bottom.home.schedule.ScheduleDialogBasicFragment.Companion.eventToEventForUpload
...
//        logToken()
    ...
    private fun checkNetworkUpload() {
        if (!NetworkManager.checkNetworkState(this)) {
            com.mongmong.namo.MainActivity.Companion.IS_MOIM_EVENT_SUCCESS = true
    ...
    private fun uploadRoomToServer() {
        ...
        for (i in unUploaded) {
            if (i.serverIdx == 0L) {
                if (i.state == com.mongmong.namo.R.string.event_current_deleted.toString()) {
                    return
                } else {
                    //POST
    ...
    private fun checkPermissions() {
        ...
        if (permissionsToRequest.isNotEmpty()) {
            ActivityCompat.requestPermissions(
                ...
                com.mongmong.namo.PERMISSION_REQUEST_CODE
            )
        } else {
            return
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == com.mongmong.namo.NOTIFICATION_PERMISSION_REQUEST_CODE) {
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == com.mongmong.namo.PERMISSION_REQUEST_CODE) {
    ...
    private fun initNavigation() {
        NavigationUI.setupWithNavController(binding.navBar,findNavController(com.mongmong.namo.R.id.nav_host))
        //바텀내비 백스택 수정 필요
    }
    ...
    override fun onPostEventSuccess(response: PostEventResponse, eventId: Long) {
        ...
        var thread = Thread{
            db.eventDao.updateEventAfterUpload(
                ...
                1,
                result.eventIdx,
                com.mongmong.namo.R.string.event_current_default.toString()
            )
        }
        thread.start()
        try {
